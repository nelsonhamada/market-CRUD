...
const Main = (): ReactElement => {
  ...
  return (
    <div className="bg-stone-800 min-h-screen flex text-indigo-400 bg-cover bg-fixed">
      <main className="bg-stone-700 grid grid-cols-3 place-items-center grid m-10 p-10 w-4/5 rounded-xl">
    { 
      isLoading ? 
        <p className="flex content-center justify-center text-4xl font-black m-3 bg-stone-700 bg-clip-text text-transparent     bg-gradient-to-r from-green-400 via-pink-500 to-purple-500 ">
          Carregando...
        </p> 
        :
          data?.results.slice(0,10).map((result) => (
            <div className="grid bg-stone-800 w-5/6 h-96 m-5 rounded-lg shadow-2xl text-ellipsis overflow-hidden" key={result.id}>
              <Link to={`/${result.id}`}>
                ...
                <section className="grid place-self-end">
